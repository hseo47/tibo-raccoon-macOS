...
import { defaultStatePaths, loadState, mutateState, StateStoreError, stateStoreTesting } from '../src/state/store';
...
afterEach(async () => { stateStoreTesting.reset(); await Promise.all(pathsToClean.splice(0).map(cleanupTempState)); });
...
describe('owner-aware locking', () => {
  ...
  test('treats EPERM ownership probes as alive', async () => {
    const statePaths = await paths(); await writeFile(statePaths.lockFile, lockRecord(999_999, new Date(Date.now() - 31_000).toISOString()), { mode: 0o600 }); stateStoreTesting.setProcessProbe(() => { throw Object.assign(new Error(), { code: 'EPERM' }); });
  ...
  test('reclaims a dead owner lock older than thirty seconds', async () => {
    const statePaths = await paths(); await writeFile(statePaths.lockFile, lockRecord(999_999, new Date(Date.now() - 31_000).toISOString()), { mode: 0o600 }); stateStoreTesting.setProcessProbe(() => { throw Object.assign(new Error(), { code: 'ESRCH' }); });
  ...
  test('does not reclaim a non-stale dead-owner lock', async () => {
    const statePaths = await paths(); await writeFile(statePaths.lockFile, lockRecord(999_999, new Date().toISOString()), { mode: 0o600 }); stateStoreTesting.setProcessProbe(() => { throw Object.assign(new Error(), { code: 'ESRCH' }); });
    await expect(mutateState(statePaths, (current) => current)).rejects.toBeInstanceOf(StateStoreError); await expect(stat(statePaths.stateFile)).rejects.toThrow();
  });
  test('times out after two seconds without changing state', async () => {
    ...
    await expect(mutateState(statePaths, (current) => ({ ...current, knownIds: ['after'] }))).rejects.toBeInstanceOf(StateStoreError); expect(Date.now() - started).toBeGreaterThanOrEqual(1_900); expect((await loadState(statePaths)).state.knownIds).toEqual(['before']);
  });
